...
export default function Dispatch() {
  ...
  const fetchActiveData = async () => {
    try {
      const jobsRes = await api.admin.getPendingBookings();
      if (jobsRes.success) setPendingJobs(jobsRes.data);
      
      const opsRes = await api.admin.getAvailableOperators();
      if (opsRes.success) setAvailableOperators(opsRes.data);
    } catch (error) {
  ...
  const handleDispatch = () => {
    ...
    setTimeout(async () => {
      try {
        await api.admin.dispatchBooking(selectedJob.id, selectedOperator.id);
        
        // Remove job locally to prevent jumping
        setPendingJobs(prev => prev.filter(j => j.id !== selectedJob.id));
        setAvailableOperators(prev => prev.filter(o => o.id !== selectedOperator.id));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full lg:h-[calc(100vh-8rem)] relative">
              ...
              >
                ...
                <div className="pt-4 border-t border-earth-dark/10/50 flex justify-between items-center">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-earth-mut uppercase tracking-widest mb-1">Schedule Time</span>
                    <span className="text-xs font-black text-earth-brown">{new Date(job.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
                  </div>
                  <Button 
                    size="sm" 
      ...
      <Card className="flex flex-col h-full bg-earth-card-alt border-earth-dark/15/50 shadow-sm rounded-[1.5rem] overflow-hidden">
        <CardHeader className="bg-earth-card/50 border-b border-earth-dark/15/50 pb-4 sm:pb-5 pt-5 sm:pt-6 px-4 sm:px-6">
          <div className="flex justify-between items-center">
            ...
            <Badge className="bg-earth-primary/10 text-earth-green border border-emerald-500/20 font-black text-[9px] sm:text-[10px] uppercase tracking-widest px-3 py-1.5">{availableOperators.length} Online</Badge>
          </div>
        </CardHeader>
          ...
          <div className={cn("transition-all duration-300", !selectedJob && "opacity-40 blur-[2px] pointer-events-none")}>
            ...
            <div className="space-y-3">
              {availableOperators.map(op => {
                ...
                const activeTractor = op.tractors?.[0] || { modelName: 'No Tractor', plateNumber: 'N/A' };
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-earth-mut uppercase tracking-widest mt-0.5">{activeTractor.modelName} <span className="mx-1">•</span> <span className="text-earth-primary">{activeTractor.plateNumber.toUpperCase()}</span></p>
                      </div>
                    </div>
                    <Button 
                      size="sm" 
                      onClick={(e) => { e.stopPropagation(); setSelectedOperator(op); handleDispatch(); }}
                      className={cn(
          ...
          {!selectedJob && (
            <div className="absolute inset-0 flex items-center justify-center p-8 bg-earth-card/10 backdrop-blur-[1px] z-20 text-center">
               <div className="bg-earth-main/80 border border-earth-dark/10 p-6 rounded-3xl shadow-2xl max-w-[280px]">
                  ...
                  <p className="text-[10px] font-bold text-earth-mut leading-relaxed uppercase">Select a pending job from the left queue to unlock the fleet for dispatching.</p>
               </div>
            </div>
          )}
        </CardContent>
      ...
      {showSuccess && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-emerald-600 text-earth-brown px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-5 animate-in slide-in-from-bottom-10 duration-500 z-[101] border border-emerald-400">
           ...
           <div>
              <p className="text-base font-black uppercase tracking-tight">Mission Dispatched</p>
              <p className="text-xs font-bold opacity-80 uppercase tracking-widest">Operator En Route to Location</p>
           </div>
