...
export default function Status() {
  ...
  const statuses = [
    { id: 'en_route', label: 'Start Journey', icon: Navigation, color: 'accent', action: 'Departing' },
    ...
    const fetchJobForStatus = async () => {
      try {
        ...
        if (res.success) {
          ...
          setActiveJob(job);
          if (job) {
            if (job.status === 'dispatched') setCurrentStatusId('Idle');
            else setCurrentStatusId(job.status);
          }
        }
      } catch (err) {
        console.error(err);
  ...
  const handleNextStep = async (statusId) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (statusId === 'en_route') setIsTimerActive(true);
